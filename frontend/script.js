...
async function handleLogin(e) {
    ...
    try {
        ...
        if (data.success) { currentUser = data.user; initApp(); }
...
function initApp() {
    ...
    if (currentUser.role === 'student') {
        menu.innerHTML = `<div class="nav-item active" onclick="navigate('dashboard', this)">📊 Dashboard</div><div class="nav-item" onclick="navigate('timetable', this)">📅 Timetable</div><div class="nav-item" onclick="navigate('attendance', this)">📈 Attendance</div>`;
        navigate('dashboard');
    } else {
        menu.innerHTML = `<div class="nav-item active" onclick="navigate('dashboard', this)">🏫 Overview</div><div class="nav-item" onclick="navigate('mark-attendance', this)">✏️ Mark Attendance</div><div class="nav-item" onclick="navigate('students', this)">👥 Students</div><div class="nav-item" onclick="navigate('shortage', this)">⚠️ Shortage List</div>`;
        navigate('dashboard');
    }
}
...
function navigate(page, el) {
    ...
    const titles = { 'dashboard': 'Dashboard', 'timetable': 'Class Timetable', 'attendance': 'Attendance Report', 'mark-attendance': 'Mark Attendance', 'students': 'Student Directory', 'shortage': 'Attendance Shortage' };
    document.getElementById('page-title').innerText = titles[page] || 'Dashboard';
    const content = document.getElementById('main-content'); content.innerHTML = ''; content.className = 'content-area fade-in'; 
...
async function loadAdminPage(page) {
    if (page === 'dashboard') {
        if(currentUser.role === 'teacher') {
            ...
            const data = await res.json();
            document.getElementById('main-content').innerHTML = `
                <div style="margin-bottom:20px;"><h2 style="margin-bottom:5px;">Good Morning, ${currentUser.name.split(' ')[1] || ''}! 👋</h2><p style="color:var(--text-muted);">Here is your schedule for <strong>${data.day}</strong>.</p></div>
